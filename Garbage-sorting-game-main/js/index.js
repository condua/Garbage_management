	...
	startInterval: function() {
		this.timer = setInterval(() => {
			if (this.total > 0) {
				...
				<p>Số điểm của bạn lần này là:${config.point}</p>
				<div>${imgtext}</div>
				<button type="button" onclick="location.reload()">Trở lại</button>
				`
				)
				$('.fail-box').css({
					display: 'flex'
	...
	this.lineMove = function() {
		...
		this.timer = setInterval(() => {
			...
			if ((this.mY > lj_box.height - config.lj_height / 2) ||
				...
				$('.wrong').html('Số lần thất bại：' + config.totao_wrong)
		...
		for (let i = 0; i < arr.length; i++) {
			if (Math.abs(arr[i].a.offsetLeft - mx) < 150 && Math.abs(arr[i].a.offsetTop - my) < 170) {
				if (arr[i].type != that.type) {
					...
					config.point += 1
					$('.point').html('Điểm số: ' + config.point)
					this.remove()
					that.stopMove()
					break;
				}
